...
func (m *kubeGenericRuntimeManager) removeContainerLog(ctx context.Context, containerID string) error {
	...
	if ok && utilfeature.DefaultFeatureGate.Enabled(features.TerminationLogFilePrune) {
		for _, mount := range status.Mounts {
			// newContainerAnnotations func in pkg/kubelet/kuberuntime/labels.go generates containerTerminationMessagePathLabel annotation
			// and makeMounts method of kubeGenericRuntimeManager in pkg/kubelet/kuberuntime/kuberuntime_container.go bind the host path with
			// container path. host path format is same with subHostPath variable. So we can use these information to recognize the termination log
			// in host.
