...
func TestWaitUntilFreshAndList(t *testing.T) {
	...
	list, resourceVersion, indexUsed, err := store.WaitUntilFreshAndList(ctx, 5, "prefix/", storage.ListOptions{Predicate: storage.Everything}, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	//}
	//list, resourceVersion, indexUsed, err = store.WaitUntilFreshAndList(ctx, 5, "prefix/", storage.ListOptions{Predicate: storage.Everything}, matchValues)
	//if err != nil {
	//	t.Fatalf("unexpected error: %v", err)
	//}
	//if resourceVersion != 5 {
...
func TestWaitUntilFreshAndListFromCache(t *testing.T) {
	...
	objs, resourceVersion, indexUsed, err := store.WaitUntilFreshAndList(ctx, 3, "prefix/", storage.ListOptions{Predicate: storage.Everything}, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if len(objs.Items) != 1 {
		t.Errorf("unexpected list returned: %#v", objs)
	}
	if indexUsed != "" {
		t.Errorf("Used index %q but expected none to be used", indexUsed)
	}
...
func TestWaitUntilFreshAndListTimeout(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, _, _, err := store.WaitUntilFreshAndList(ctx, 4, "", storage.ListOptions{Predicate: storage.Everything}, nil)
...
func TestReflectorForWatchCache(t *testing.T) {
	...
	{
		_, version, _, err := store.WaitUntilFreshAndList(ctx, 0, "", storage.ListOptions{Predicate: storage.Everything}, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if version != 0 {
	...
	{
		_, version, _, err := store.WaitUntilFreshAndList(ctx, 10, "", storage.ListOptions{Predicate: storage.Everything}, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if version != 10 {
