...
func TestReflectorHandleWatchStoppedBefore(t *testing.T) {
	...
	err := handleWatch(time.Now(), fw, s, g.expectedType, g.expectedGVK, g.name, g.typeDescription, g.setLastSyncResourceVersion, g.clock, nevererrc, stopCh, g.metrics)
	if err == nil {
...
func TestReflectorHandleWatchResultChanClosedBefore(t *testing.T) {
	...
	close(resultCh)
	err := handleWatch(time.Now(), fw, s, g.expectedType, g.expectedGVK, g.name, g.typeDescription, g.setLastSyncResourceVersion, g.clock, nevererrc, wait.NeverStop, g.metrics)
	if err == nil {
		t.Errorf("unexpected non-error")
	}
	// Ensure the watcher methods are called exactly once in this exact order.
...
func TestReflectorWatchHandler(t *testing.T) {
	...
	err := handleWatch(time.Now(), fw, s, g.expectedType, g.expectedGVK, g.name, g.typeDescription, setLastSyncResourceVersion, g.clock, nevererrc, stopCh, g.metrics)
...
func TestReflectorStopWatch(t *testing.T) {
	...
	close(stopWatch)
	err := handleWatch(time.Now(), fw, s, g.expectedType, g.expectedGVK, g.name, g.typeDescription, g.setLastSyncResourceVersion, g.clock, nevererrc, stopWatch, g.metrics)
